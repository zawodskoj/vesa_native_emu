...
pattern_t patterns[] = {
    ...
    pattern_t(selectByRangeIx({ "add", "or", "adc", "sbb", "and", "sub", "xor", "cmp" }), { encodedAluOpcode(0x80) })
        ...
        .ACT(LHS.set(alu.dispatch(ALU_OP80, args[2].deref(), LHS, RHS))), // BUG: cmp modifies LHS value
    ...
    pattern_t("mov", { sizeAndOrderMarked(0x88) })
        ...
        .ACT(LHS.set(RHS)),
    pattern_t("mov", { 0x8c })
        .withArgs({ modrmReference, modrmSegRegister }) // todo check: only register or memory reference, no r/m
        .ACT(LHS.set(RHS)),
